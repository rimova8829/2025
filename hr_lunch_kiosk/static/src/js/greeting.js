odoo.define('hr_lunch_kiosk.greeting_message', function (require) {
"use strict";
...
var GreetingMessage = AbstractAction.extend({
    contentTemplate: 'HrAttendanceLunchGreetingMessage',
    ...
    init: function(parent, action) {
        ...
        if(!action.attendance) {
            ...
            this.getSession().user_has_group('hr_attendance.group_hr_attendance_user').then(function(has_group) {
                if(has_group) {
                    self.next_action = 'hr_lunch_kiosk.hr_attendance_action_kiosk_lunch_mode';
                } else {
        ...
        if (this.next_action != 'hr_lunch_kiosk.hr_attendance_action_kiosk_lunch_mode' && this.next_action.tag != 'hr_attendance_kiosk_mode') {
            this.activeBarcode = false;
        }

        this.attendance = action.attendance;
        this.emp_data = action.emp_data;
        ...
        this.attendance.lunch_start = this.attendance.lunch_start && moment.utc(this.attendance.lunch_start).local();
        this.attendance.lunch_end = this.attendance.lunch_end && moment.utc(this.attendance.lunch_end).local();
        ...
        this.attendance.lunch_start_time = this.attendance.lunch_start && this.attendance.lunch_start.format(this.format_time);
        this.attendance.lunch_end_time = this.attendance.lunch_end && this.attendance.lunch_end.format(this.format_time);
...
core.action_registry.add('hr_attendance_lunch_greeting_message', GreetingMessage);
